...
export default function ClassPage() {
  const [catergory, setCategory] = useState('전체')
  const [selectedTitle, setSelectedTitle] =useState('')

  /* 필터링 */
  const filtered =
    catergory === "전체"// 1.조건: 선택된 카테고리가 '전체'인가 
      ? classes //2. true이면 원본데이터 전체를 보여줘.
      : classes.filter(item => item.category === catergory)
    ...
    <section className='sec-classpage'>
      <div className="inner">
        ...
        <div className="section-box">
          <h3>선택한 강의</h3>
          <p>
            {selectedTitle ? `${selectedTitle} 강의를 선택했습니다.` : '아직 산택한 강의가 없습니다.'}
          </p>
        </div>
      </div>
    </section>
